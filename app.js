...
var App = (() => {
  ...
  function resolveFoul(defendingSide, attackingSide, fouler, victim, nearBox, forcePenalty) {
    ...
    if (nearBox && (forcePenalty || Math.random() < 0.09)) {
  ...
  function maybeSecondaryMatchEvent() {
    ...
    if (roll < 0.24) {
      ...
      if (fouler) {
        ...
        if (result && result.outcome !== 'red' && result.outcome !== 'penalty' && Math.random() < 0.35) {
      ...
      if (p) {
        const nearBox = Math.random() < 0.45;
        const givenAsPen = nearBox && Math.random() < 0.22;
        if (givenAsPen) {
  ...
  function decideOpeningTactic(selfStr, oppStr, style) {
    ...
    if (gap <= -4) return Math.random() < 0.6 ? 'defend' : 'balanced';
    if (gap >= 5) return Math.random() < (possessionMinded ? 0.65 : 0.5) ? (possessionMinded ? 'press' : 'attack') : 'balanced';
    if (counterMinded && gap < 2) return Math.random() < 0.35 ? 'defend' : 'balanced';
    if (possessionMinded) return Math.random() < 0.4 ? 'press' : 'balanced';
    return 'balanced';
  }
  ...
  function evaluateTacticalAI(side, otherSide) {
    ...
    if (diff <= -1 && minute >= 60) {
      ...
      targetTac = Math.random() < 0.35 ? 'press' : currentTac;
    } else if (diff === 0 && minute < 60 && currentTac !== 'balanced' && Math.random() < 0.1) {
